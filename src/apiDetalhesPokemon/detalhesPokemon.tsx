...
export default function detalhesPokemon() {
  ...
  async function mudarSpriteGen(gen: number) {
    const response = await fetch(`https://pokeapi.co/api/v2/pokemon/${dadosNome}`);
    const data = await response.json();
    ...
    if (spritePath) {
        const sprite = spritePath.split('.').reduce((o, i) => o[i], data.sprites.versions);
    ...
    <>
      <div className={style.detailsPokemonScreen}>
        <div className={style.infoScreen}>
          ...
          <h2>Name : {dadosNome.charAt(0).toUpperCase() + dadosNome.slice(1)}</h2>
          <h2>Image :</h2>
          <br />
          ...
          <div className={style.infoScreenGenButtons}>
            ...
            <Button variant="contained" onClick={() => mudarSpriteGen(1)} color="primary">1 gen </Button>
            <Button variant="contained" onClick={() => mudarSpriteGen(2)} color="primary">2 gen </Button>
            <Button variant="contained" onClick={() => mudarSpriteGen(3)} color="primary">3 gen </Button>
            <Button variant="contained" onClick={() => mudarSpriteGen(4)} color="primary">4 gen </Button>
            <Button variant="contained" onClick={() => mudarSpriteGen(5)} color="primary">5 gen </Button>
            <Button variant="contained" onClick={() => mudarSpriteGen(6)} color="primary">6 gen </Button>
            <Button variant="contained" onClick={() => mudarSpriteGen(7)} color="primary">7 gen </Button>
          </ThemeProvider>
          </div>
          <h2>Types :</h2>
